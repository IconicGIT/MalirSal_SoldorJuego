...
SoldorEnemy::~SoldorEnemy() {}

bool SoldorEnemy::Awake()
{
	return false;
}


bool SoldorEnemy::Start()
{
	state = STATE_WAIT;
	speed = 1;
	Vspeed = { speed,speed };

	pac = app->tex->Load("Assets/textures/enemies/Camello.png");
	LifeBars = app->tex->Load("Assets/textures/UI/HealthBar DARK.png");

	recHealth = { 18, 152, 59, 6 };
	recHealthBG = { 17, 164, 61, 8 };

	idle.PushBack({ 0, 128 * 2, 144, 128 });
	idle.PushBack({ 144 * 1, 128 * 2, 144, 128 });
	idle.PushBack({ 144 * 2, 128 * 2, 144, 128 });
	idle.PushBack({ 144 * 3, 128 * 2, 144, 128 });
	idle.PushBack({ 144 * 4, 128 * 2, 144, 128 });
	idle.PushBack({ 144 * 5, 128 * 2, 144, 128 });
	idle.PushBack({ 144 * 6, 128 * 2, 144, 128 });
	idle.PushBack({ 144 * 7, 128 * 2, 144, 128 });
	idle.PushBack({ 144 * 8, 128 * 2, 144, 128 });
	idle.PushBack({ 144 * 9, 128 * 2, 144, 128 });
	idle.PushBack({ 144 * 10, 128 * 2, 144, 128 });
	idle.PushBack({ 144 * 11, 128 * 2, 144, 128 });
	idle.PushBack({ 144 * 12, 128 * 2, 144, 128 });
	idle.PushBack({ 144 * 13, 128 * 2, 144, 128 });
	//idle.PushBack({ 144 * 14, 128 * 2, 144, 128 });
	idle.speed = 0.15f;
...
void SoldorEnemy::Draw()
{
	...
	app->render->DrawTexture(LifeBars, METERS_TO_PIXELS(Hitbox->body->GetPosition().x) - 30, METERS_TO_PIXELS(Hitbox->body->GetPosition().y) - 61, &recHealthBG);
